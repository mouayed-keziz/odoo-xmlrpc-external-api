...
async function create_job_application(name, partner_name, email, phone, job_id, type_id, file_path) {
    ...
    console.log(updated_application_id)

    try {
        const application = await odoo.execute_kw('hr.applicant', "search_read", [])
        //stringify teh result and put it in output.txt
        fs.writeFileSync("output.txt", JSON.stringify(application, null, 2))
    } catch (error) {
        console.log("Error while reading application 4")
        exit()
    }
...
const job_id = 11
const name = "test 3"
const partner_name = "test 3"
const email = "test.3test.com"
const phone = "333333333"
const type_id = 1
const file_path = "./files/test2.pdf"
